...
@Data
public class LineDiffInstrumentVisitor extends TraversalVisitor {

	private final static String __name__ = "@LineDiffInstrumentVisitor ";
	...
	public boolean visit(MethodDeclaration node) {
		...
		if (node.getBody() != null) {
			...
			if (instrumentSet.contains(message)) {
				...
				// Statement endGuard = GenStatement.genDumpLine(writeFile, message +
				// Constant.INSTRUMENT_END, 0 );
				...
				for (Object object : methodBody.statements()) {
					if (object instanceof Statement) {
						...
						blockStatement.add(ASTNode.copySubtree(ast, (ASTNode) object));
					}
				}
	...
	private List<Statement> process(Statement statement, String message) {
		...
		if (statement instanceof IfStatement) {
			...
			Statement insert = StatementGenerator.genDumpLine(writeFile, message, lineNumber);
			result.add(insert);
	...
	public static void main(String[] args) {
		String filePath = "/projects/Chart/Chart_1_buggy/source/org/jfree/chart/renderer/category/AbstractCategoryItemRenderer.java";
		LineDiffInstrumentVisitor statementInstrumentVisitor = new LineDiffInstrumentVisitor();
		statementInstrumentVisitor.setIntrumentMethod(
				"org.jfree.chart.renderer.categoryAbstractCategoryItemRenderer#LegendItemCollection#getLegendItems#?");
