...
public class PatchRun {
	...
	public static void runForMath30() {
		...
		for (File file : new File(initialPatchPath).listFiles()) {
			...
			String diffFile = "/Users/liangjingjing/WorkSpace/Project/PatchFilterBackUp/PatchBackup/FinalPatch/Math/30/"
					+ file.getName();
	...
	public static void samePostion() {
		String name = "Math";
		String id = "41";
		String initialPatchPath = "/Users/liangjingjing/WorkSpace/Project/PatchFilterBackUp/PatchBackup/TotalPatch/"
				+ name + "/" + id;
		...
		for (File file : new File(initialPatchPath).listFiles()) {
			...
			keyBuffer.append(content[0].split("\\.java")[0]).append(content[1].split("\\.patched")[0])
					.append(content[2]).append(3);
	...
	public static void changeFor1Line() {
		...
		for (File file : new File(initialPatchPath).listFiles()) {
			...
			/*
			 * for(String line: content.split("\n")) { if(line.startsWith("+") && !
			 * line.startsWith("+++")) { replaceLine = line.split("\\+")[1]; break; } }
			 */
			try {
				FileUtils.copyFile(new File(replaceFile), new File(replaceFile + ".patched"));
			} catch (IOException e) {
			...
			patchedCon[replaceLineNumber - 1] = replaceLine;
			/*
			 * while (index > 0) { patchedCon[replaceLineNumber-1 +index] =
			 * "woshilinshibianliang"; index--; }
			 */
			StringBuffer resultBuffer = new StringBuffer();
			for (int i = 0; i < patchedCon.length; i++) {
			...
			FileIO.writeStringToFile(replaceFile + ".patched", resultBuffer.toString(), false);
			String diffFile = "/Users/liangjingjing/WorkSpace/Project/PatchFilterBackUp/PatchBackup/FinalPatch/" + name
					+ "/" + id + "/" + file.getName();
			Runner.diff2File(replaceFile, replaceFile + ".patched", diffFile);
		}
	}
	...
	public static void moveCardumenPatch() {
		...
		for (File file : new File(originalDir).listFiles()) {
			...
			Project subject = new Project(project, Integer.parseInt(id));
			...
			/*
			 * try { FileUtils.copyFile(file, new File(targetDir + patchName)); } catch
	...
	public static void generalRun() {
		String sourceFile = "/home/xushicheng/patchrun/origin/Math_41_buggy/src/main/java/org/apache/commons/math/stat/descriptive/moment/Variance.java";
		String targetFile = "/home/xushicheng/patchrun/fixed/Math_41_buggy/src/main/java/org/apache/commons/math/stat/descriptive/moment/Variance.java";
		String patchFile = "/home/xushicheng/patchrun/fixed/Cardumen-patch1";
		Runner.diff2File(sourceFile, targetFile, patchFile);
	}

	public static boolean install() {
		String targetFile = "/home/xushicheng/patchrun/fixed/Math_41_buggy/src/main/java/org/apache/commons/math/stat/descriptive/moment/Variance.java";
		String patchFile = "/home/xushicheng/patchrun/fixed/Cardumen-patch1";
		System.out.println("Patch To File");
		if (Runner.patchFile(targetFile, patchFile)) {
			return true;
		}
		return false;
	}
