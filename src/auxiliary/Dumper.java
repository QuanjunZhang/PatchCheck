...
public class Dumper {
	...
	private final static long MAX_OUTPUT_FILE_SIZE = 1024;
	private final static int MAX_DEPTH = 3;
	private final static int ARRAY_MAX_LENGTH = 5;
	// private final static String OUT_AND_LIB_PATH =
	// "/Users/Jiajun/Code/Java/fault-localization/StateBasedFL";
	// private final static String OUT_FILE_NAME = OUT_AND_LIB_PATH +
	// "/out/path.out";
	private final static String LIB_FILE_PATH = "/home/xushicheng/eclipse-workspace/PatchCheck/src//auxiliary/lib";
	...
	public static Object write(String writeFile, String VarName, Object objValue) {
		if (objValue == null) {
			return objValue;
		}
		...
		try {
			...
			bufferedWriter.write(dump(objValue));
	...
	public static int write(String writeFile, String VarName, int intValue) {
		File file = new File(writeFile);
		if (!file.exists()) {
			try {
				file.getParentFile().mkdirs();
		...
		try {
			bufferedWriter.write(VarName);
			bufferedWriter.write(dump(intValue));
			bufferedWriter.write("\n");
			bufferedWriter.close();
		} catch (IOException e) {
			e.printStackTrace();
	...
	public static boolean write(String writeFile, String VarName, boolean boolValue) {
		File file = new File(writeFile);
		if (!file.exists()) {
			try {
				file.getParentFile().mkdirs();
		...
		try {
			bufferedWriter.write(VarName);
			bufferedWriter.write(dump(boolValue));
			bufferedWriter.write("\n");
			bufferedWriter.close();
		} catch (IOException e) {
			e.printStackTrace();
	...
	public static boolean write(String writeFile, String strValue) {
		if (strValue == null) {
			return false;
		}
		File file = new File(writeFile);
	...
	public static String dump(Object val) {
		return dump(val, MAX_DEPTH, ARRAY_MAX_LENGTH);
	}

	public static String dump(int val) {
		return dump(Integer.valueOf(val), MAX_DEPTH, ARRAY_MAX_LENGTH);
	}

	public static String dump(boolean val) {
		return dump(Boolean.valueOf(val), MAX_DEPTH, ARRAY_MAX_LENGTH);
	}

	public static String dump(float val) {
		return dump(Float.valueOf(val), MAX_DEPTH, ARRAY_MAX_LENGTH);
	}

	public static String dump(double val) {
		return dump(Double.valueOf(val), MAX_DEPTH, ARRAY_MAX_LENGTH);
	}

	public static String dump(long val) {
		return dump(Long.valueOf(val), MAX_DEPTH, ARRAY_MAX_LENGTH);
	}

	public static String dump(byte val) {
		return dump(Byte.valueOf(val), MAX_DEPTH, ARRAY_MAX_LENGTH);
	}

	public static String dump(char val) {
		return dump(Character.valueOf(val), MAX_DEPTH, ARRAY_MAX_LENGTH);
	}
	...
	protected static Object dump(Object o, DumpContext ctx) {
		...
		if (oClass.isArray()) {
			...
			List<Object> elemets = new ArrayList<Object>();
