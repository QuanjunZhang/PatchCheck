...
public class MethodTraceAnalysis {
	...
	public void MainProcess() {
		...
		HashMap<String, List<Patch>> traceMap = new LinkedHashMap<String, List<Patch>>();
		for(Patch patchFile: patchFiles) {
			...
			if(traceMap.containsKey(trace)) {
				...
				List<Patch> tmpList = new LinkedList<Patch>();
	...
	public void findDifferenceTraces(HashMap<String, List<Patch>> traceMap) {
		//  颜色标识 不同的trace
		if(traceMap.size() == 1) {		
			
		}else if(traceMap.size() == 2) {   // 两组 trace 不同，需要找到第一条不同的语句
	        List<List<Patch>> list = new ArrayList<>(traceMap.values());
